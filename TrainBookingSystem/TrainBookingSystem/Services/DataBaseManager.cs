...
namespace TrainBookingSystem.Services
{
    public class DataBaseManager
    {
        ...
        public void UpdateTrip(int TripID = 0, int TrainID = 0, String source = "", String destination = "", String date = "", String arrivalDate = "")
        {
            // flag
            bool inserted = false;
            ...
            try
            {
                ...
                // query
                String query = "UPDATE Trip SET TrainID = @TrainID, Source = @Source, Destination = @Destination, ArrivalTime = @ArrivalTime, DepartureTime = @DepartureTime WHERE TripID= @TripID";
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@TripID", TripID);
                    command.Parameters.AddWithValue("@TrainID", TrainID);
                    command.Parameters.AddWithValue("@Source", source);
                    command.Parameters.AddWithValue("@Destination", destination);
                    command.Parameters.AddWithValue("@DepartureTime", date);
                    command.Parameters.AddWithValue("@ArrivalTime", arrivalDate);
